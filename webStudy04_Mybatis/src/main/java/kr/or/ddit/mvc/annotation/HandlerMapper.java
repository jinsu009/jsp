...
public class HandlerMapper implements IHandlerMapper {
	...
	public HandlerMapper(String...basePackages) {
		...
		while(it.hasNext()) {
			...
			try {
				handlerObj = handlerClass.newInstance(); // 핸들러 객체 생성 
			}catch(Exception e) {
				logger.error(e.getMessage() ,e);
				continue;
			}
			Map<Method,Annotation> methodMap = 
					ReflectionUtils.getMethodsWithAnnotationAtClass(handlerClass, 
							annotationType, String.class);
			for(Entry<Method, Annotation> entry : methodMap.entrySet()) {
				...
				logger.info("{}:{}",condition,mappingInfo); // 정상처리 = info
			}
		}
	}
